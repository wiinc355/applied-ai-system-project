...
## Title and Summaryrecs → humancheck[Human reviews recommendations]
...
2. **Create and activate a virtual environment**
   ```bash
   python -m venv .venv
   source .venv/bin/activate        # macOS /        # Windows
   ```
...
## Reflection 
I’ll provide a polished reflection paragraph you can paste directly into your project submission.This project taught me that AI and problem-solving are tightly connected: building a model is not just about getting outputs, but about making trade-offs, testing assumptions, and verifying behavior under real conditions. I learned that a system can be reliable in one sense (stable, repeatable outputs) and still fail user intent when data coverage or feature weighting is limited. The most important lesson was that human judgment is essential at every stage: AI tools can speed up coding and generate ideas, but meaningful progress came from evaluating results critically, measuring performance with clear metrics, and iterating on the design based on evidence rather than intuition alone.
